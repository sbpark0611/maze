...
from envs.memory_maze import tasks
from utils.misc import linear_schedule
from ppo.epn import EPNPPO
from stable_baselines3.common.save_util import load_from_zip_file

from envs.cartpole_wrapper import vision_cart_pole

def create_env():
    return tasks.memory_maze_9x9()


@hydra.main(config_path="./ppo/epn", config_name="config_3d")
def main(cfg: DictConfig) -> None:
    env_kwargs = dict(cfg.env)

    env = make_vec_env(create_env, n_envs=cfg.env.n_envs)
    env = VecNormalize(env, norm_obs=cfg.env.norm_obs, norm_obs_keys=["image", "prev_image"], norm_reward=cfg.env.norm_reward)

    eval_env = make_vec_env(
        create_env, n_envs=cfg.env.n_envs
    )
    eval_env = VecNormalize(
        eval_env, norm_obs=cfg.env.norm_obs, norm_obs_keys=["image", "prev_image"], norm_reward=cfg.env.norm_reward
    )
