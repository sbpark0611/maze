...
class MemoryMazeTask(random_goal_maze.NullGoalMaze):
    ...
    def __init__(self,
        ...
        self.pick_random_target = pick_random_target
    ...
    def initialize_episode(self, physics, rng: RandomState):
        #print("initialize_episode")
        super().initialize_episode(physics, rng)
        self._rewarded_this_step = False
        self._targets_obtained = 0

        self._num_step = 0
        self.is_color_swapped = False

    def after_step(self, physics, rng: RandomState):
        #print("after_step", self._num_step + 1)
        ...
        self._num_step += 1
        if self._num_step == 500:
            if self.reverse_mode == 1:
                self._swap_position_of_target(rng, self.pick_random_target)
            elif self.reverse_mode == 3:
                self._swap_color_of_target(rng, self.pick_random_target)
    ...
    def _swap_position_of_target(self, rng: RandomState, pick_random_target):
        #print("swap position")
        ix = self._random_or_farthest(rng, pick_random_target)

        # swap position
        current_target_position = mjcf.get_attachment_frame(self._targets[self._current_target_ix].mjcf_model).pos
        rand_target_position = mjcf.get_attachment_frame(self._targets[ix].mjcf_model).pos
        mjcf.get_attachment_frame(self._targets[self._current_target_ix].mjcf_model).pos = rand_target_position
        mjcf.get_attachment_frame(self._targets[ix].mjcf_model).pos = current_target_position

    def _swap_color_of_target(self, rng: RandomState, pick_random_target):
        #print("swap color")
        ix = self._random_or_farthest(rng, pick_random_target)

        self.target_color_before_swap = self._target_colors[self._current_target_ix]

        self._target_colors[ix], self._target_colors[self._current_target_ix] = \
            self._target_colors[self._current_target_ix], self._target_colors[ix] # swap color
        
        # save current target positions
        positions = []
        for target in self._targets:
            positions.append(mjcf.get_attachment_frame(target.mjcf_model).pos)

        # create targets with new color
        self._create_targets(clear_existing=True, randomize_colors=False, rng=rng)

        # place targets as it was
        for target, pos in zip(self._targets, positions):
            mjcf.get_attachment_frame(target.mjcf_model).pos = pos

        self.is_color_swapped = True

    def _random_or_farthest(self, rng: RandomState, pick_random_target):
        if pick_random_target:
            while True:
                ix = rng.randint(len(self._targets))
                if self._current_target_ix == ix:
                    continue # pick random target which is not current target
                break
        else:
            cur_pos = mjcf.get_attachment_frame(self._targets[self._current_target_ix].mjcf_model).pos
            max_dis = -1
            for i, target in enumerate(self._targets):
                target_pos = mjcf.get_attachment_frame(target.mjcf_model).pos
                d = self._distance(cur_pos, target_pos)
                if max_dis < d:
                    ix = i
                    max_dis = d
        return ix
