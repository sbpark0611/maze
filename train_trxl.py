...
def main(cfg: DictConfig) -> None:
    ...
    #         "linear_schedule",
    #     ]:
    #         loaded_config.__dict__[x] = config.__dict__[x]
    #     config = deepcopy(loaded_config)
    #     print("RUN DIR:", config.run_dir)
    # if config.load_path is not None:
    #     env_kwargs["set_cues"] = loaded_maps[: config.n_maps]
    ...
    #     _, params, _ = load_from_zip_file(cfg.load_path)
    #     model.policy.load_state_dict(params["policy"], strict=False)
    #     # model.policy.model.load_state_dict(model_state_dict)
    #     model.finetune_mode()
