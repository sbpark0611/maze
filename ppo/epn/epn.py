...
class EPNPPO(OnPolicyAlgorithm):
    policy_aliases: ClassVar[Dict[str, Type[BasePolicy]]] = {
        "ActorCriticEpnPolicy": ActorCriticEpnPolicy,
    }
    ...
    def _setup_model(self) -> None:
        ...
        if not isinstance(self.policy, ActorCriticEpnPolicy):
            raise ValueError("Policy must subclass ActorCriticEpnPolicy")

        self.rollout_buffer = EPNDictRolloutBuffer(
            self.n_steps,
            self.observation_space,
            self.action_space,
            self.device,
    ...
    def collect_rollouts(
        ...
        rollout_buffer: EPNDictRolloutBuffer,
        n_rollout_steps: int,
    ) -> bool:
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                memory = rollout_buffer.episodic_storage()
            ...
            if dones.any() and infos[0].get("terminal_observation") is not None:
                ...
                for key, val in memory.items():
                    _val = deepcopy(val)
                    _val[:, -1] = obs_tensor[key]
                    terminal_memory[key] = _val

                with th.no_grad():
                    terminal_value = self.policy.predict_values(
                        terminal_memory, terminal_obs
                    )[..., 0]
        ...
        with th.no_grad():
            ...
            memory = rollout_buffer.episodic_storage()
            values = self.policy.predict_values(memory, new_obs_tensor)
